...
class GraphVisualizer:
    ...
    def visualize_network(
        self, filename: str = "output/village_network.png", show: bool = False
    ) -> None:
        ...
        for village in self.graph.get_all_villages():
            for neighbor, weight in self.graph.get_neighbors(village):
                ...
                if edge not in drawn_edges:
                    ...
                    plt.plot(
                        [x1, x2], [y1, y2], "gray", linewidth=2, alpha=0.6, zorder=1
                    )
        ...
        for village_id in self.graph.get_all_villages():
            ...
            circle = plt.Circle(
                (x, y), 0.3, color="lightblue", ec="black", linewidth=2, zorder=3
            )
            ax.add_patch(circle)
    ...
    def visualize_path(
        self,
        path: List[int],
        filename: str = "output/shortest_path.png",
        show: bool = False,
        ...
        for village_id in self.graph.get_all_villages():
            ...
            circle = plt.Circle(
                (x, y), 0.3, color=color, ec="black", linewidth=2, zorder=4
            )
            ax.add_patch(circle)
    ...
    def create_multiple_visualizations(
        self, routes_with_paths: List[tuple], prefix: str = "route"
    ) -> None:
